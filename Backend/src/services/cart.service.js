...
const getCartWithDetails = async (userId) => {
    const cart = await Cart.findOne({ userId })
        .populate({
            ...
            populate: [
                {
                    path: 'storeId',
                    select: 'storeIdentity.name storeIdentity.slug storeStatus',
                },
    ...
    prices.forEach((p) => {
        ...
        if (
            !priceMap.has(id) ||
            p.effectiveFrom > priceMap.get(id).effectiveFrom
        ) {
            priceMap.set(id, p);
        }
    });
...
const updateCartItem = async (userId, productId, quantity) => {
    ...
    if (!inventory || inventory.availableStock < quantity) {
        throw new ApiError(
            400,
            `Only ${inventory?.availableStock || 0} items available`
        );
    }
    ...
    for (const item of cart.items) {
        ...
        if (!item.availability?.isAvailable) {
            if (item.availability?.availableQuantity === 0) {
                ...
                itemIssues.push(
                    `Only ${item.availability.availableQuantity} available`
                );
            }
        }
