...
export const getStoreOverview = async (storeId, dateRange = 30) => {
    ...
    const [orderStats, productStats, revenueByDay, topProducts] =
        await Promise.all([
            // Order statistics
            Order.aggregate([
                {
    ...
    orderStats.forEach((stat) => {
        orderSummary.byStatus[stat._id] = {
            count: stat.count,
            revenue: stat.revenue,
        };
        orderSummary.total += stat.count;
        if (
            ![ORDER_STATUS.CANCELLED, ORDER_STATUS.REFUNDED].includes(stat._id)
        ) {
...
export const generateDailyMetrics = async (storeId, date) => {
    ...
    const orderData = result.orderMetrics[0] || {
        totalOrders: 0,
        totalRevenue: 0,
        uniqueCustomers: [],
    };
