 ...
 * Create a new store
 */
const createStore = async (sellerId, storeData, ipAddress = null, userAgent = null) => {
...
const getStoreBySlug = async (slug) => {
  const store = await Store.findOne({ "storeIdentity.slug": slug })
 ...
 * Update store
 */
const updateStore = async (storeId, sellerId, updateData, ipAddress = null, userAgent = null) => {
  const store = await Store.findById(storeId);
  if (!store) {
    throw new ApiError(404, "Store not found");
  }
  ...
  if (store.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to update this store");
  }
  ...
  if (updateData.name) {
    ...
    updates["storeIdentity.slug"] = await ensureUniqueSlug(baseSlug, storeId);
  }
 ...
 * Suspend store (Admin)
 */
const suspendStore = async (storeId, adminId, reason, ipAddress = null, userAgent = null) => {
  if (!reason) {
 ...
 */
const reactivateStore = async (storeId, adminId, ipAddress = null, userAgent = null) => {
  const store = await Store.findById(storeId);
  if (!store) {
    throw new ApiError(404, "Store not found");
  }
  ...
  if (!seller || seller.status !== SELLER_STATUS.APPROVED) {
    throw new ApiError(400, "Cannot reactivate store - seller is not approved");
  }
 ...
 * Close store (Seller)
 */
const closeStore = async (storeId, sellerId, ipAddress = null, userAgent = null) => {
  const store = await Store.findById(storeId);
  if (!store) {
    throw new ApiError(404, "Store not found");
  }
  ...
  if (store.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to close this store");
  }
