 ...
 */
const createProduct = async (sellerId, storeId, productData, ipAddress = null, userAgent = null) => {
  const { identity, categoryId, attributes, price, images, initialStock = 0 } = productData;
  ...
  if (store.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to add products to this store");
  }
...
const getProductWithDetails = async (productId) => {
  const product = await Product.findById(productId)
    .populate("storeId", "storeIdentity.name storeIdentity.slug storeStatus")
  ...
  // Get images
  const images = await ProductImage.find({ productId }).sort({ sortOrder: 1 });
 ...
 * Update product
 */
const updateProduct = async (productId, sellerId, updateData, ipAddress = null, userAgent = null) => {
  ...
  if (updateData.identity) {
    ...
    if (updateData.identity.description) {
      updates["identity.description"] = updateData.identity.description.trim();
    }
  }
 ...
 * Publish product
 */
const publishProduct = async (productId, sellerId, ipAddress = null, userAgent = null) => {
  const product = await Product.findById(productId).populate("storeId");
  if (!product) {
    throw new ApiError(404, "Product not found");
  }
  ...
  if (product.storeId.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to publish this product");
  }
 ...
 * Archive product
 */
const archiveProduct = async (productId, sellerId, ipAddress = null, userAgent = null) => {
  const product = await Product.findById(productId).populate("storeId");
  if (!product) {
    throw new ApiError(404, "Product not found");
  }
  ...
  if (product.storeId.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to archive this product");
  }
 ...
 */
const deleteProduct = async (productId, sellerId, ipAddress = null, userAgent = null) => {
...
const getProductsByStore = async (storeId, options = {}) => {
  ...
  prices.forEach(p => {
    if (!priceMap.has(p.productId.toString()) || 
  ...
  if (minPrice !== undefined || maxPrice !== undefined) {
    filteredProducts = enrichedProducts.filter(p => {
      ...
      if (minPrice !== undefined && p.currentPrice.price < minPrice) return false;
      if (maxPrice !== undefined && p.currentPrice.price > maxPrice) return false;
      return true;
    });
  }
  ...
  if (inStock !== undefined) {
    filteredProducts = filteredProducts.filter(p => {
      ...
      return inStock ? p.inventory.availableStock > 0 : p.inventory.availableStock === 0;
    });
  }
...
const searchProducts = async (options = {}) => {
  ...
  const activeStores = await Store.find({ storeStatus: STORE_STATUS.ACTIVE }).select("_id");
  const activeStoreIds = activeStores.map(s => s._id);
  query.storeId = query.storeId ? query.storeId : { $in: activeStoreIds };

  return getProductsByStore(null, {
    ...options,
...
const getPriceHistory = async (productId, sellerId = null) => {
  ...
  if (sellerId && product.storeId.sellerId.toString() !== sellerId.toString()) {
    throw new ApiError(403, "You don't have permission to view this product's price history");
  }
