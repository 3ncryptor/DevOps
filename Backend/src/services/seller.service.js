 ...
 */
const registerSeller = async (userId, sellerData, ipAddress = null, userAgent = null) => {
  const { business, contact } = sellerData;

  // Validate required fields
  if (!business?.legalName || !business?.displayName || !business?.businessType) {
    throw new ApiError(400, "Business legal name, display name, and type are required");
  }
...
const getSellerByUserId = async (userId) => {
  const seller = await Seller.findOne({ userId })
    .populate("userId", "email accountStatus");
 ...
 */
const updateSeller = async (sellerId, userId, updateData, ipAddress = null, userAgent = null) => {
  const seller = await Seller.findById(sellerId);
  if (!seller) {
    throw new ApiError(404, "Seller not found");
  }
  ...
  const allowedUpdates = ["business.displayName", "business.taxIdentifier", "contact.phone"];
  const updates = {};
  const previousState = seller.toObject();

  // Build update object
  if (updateData.business?.displayName) {
    updates["business.displayName"] = updateData.business.displayName.trim();
  }
  if (updateData.business?.taxIdentifier !== undefined) {
    updates["business.taxIdentifier"] = updateData.business.taxIdentifier?.trim();
  }
 ...
 */
const submitVerificationDocuments = async (sellerId, documents, ipAddress = null, userAgent = null) => {
  const seller = await Seller.findById(sellerId);
  if (!seller) {
    throw new ApiError(404, "Seller not found");
  }
 ...
 */
const approveSeller = async (sellerId, adminId, notes = null, ipAddress = null, userAgent = null) => {
  const seller = await Seller.findById(sellerId);
  if (!seller) {
    throw new ApiError(404, "Seller not found");
  }
 ...
 * Reject seller (Admin)
 */
const rejectSeller = async (sellerId, adminId, reason, ipAddress = null, userAgent = null) => {
  if (!reason) {
 ...
 */
const suspendSeller = async (sellerId, adminId, reason, ipAddress = null, userAgent = null) => {
  if (!reason) {
 ...
 */
const reactivateSeller = async (sellerId, adminId, ipAddress = null, userAgent = null) => {
  const seller = await Seller.findById(sellerId);
  if (!seller) {
    throw new ApiError(404, "Seller not found");
  }
...
const getSellerWithDetails = async (sellerId) => {
  const seller = await Seller.findById(sellerId)
    .populate("userId", "email accountStatus createdAt lastLoginAt");
