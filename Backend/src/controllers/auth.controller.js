...
const login = asyncHandler(async (req, res) => {
  const { ipAddress, userAgent } = getClientInfo(req);
  const { user, accessToken, refreshToken } = await loginUser(req.body, ipAddress, userAgent);
...
const refresh = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, { user, accessToken }, "Token refreshed successfully"));
});
...
const logoutAll = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, result, "Logged out from all devices"));
});
...
const changePasswordHandler = asyncHandler(async (req, res) => {
  ...
  await changePassword(req.user.id, currentPassword, newPassword, ipAddress, userAgent);
