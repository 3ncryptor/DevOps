...
export const getPlatformStats = asyncHandler(async (req, res) => {
    ...
    const [userGrowth, orderTrend, topStores, topProducts] = await Promise.all([
...
export const getUserHandler = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(200, result, 'User retrieved successfully')
    );
});
...
export const activateUserHandler = asyncHandler(async (req, res) => {
    ...
    const user = await activateUser(userId, req.user.id, ipAddress, userAgent);
...
export const getAllCategories = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(
            200,
            { categories: roots, flat: categories },
...
export const getAuditLogsHandler = asyncHandler(async (req, res) => {
    const { page, limit, action, userId, targetType, startDate, endDate } =
        req.query;
...
export const suspendStoreHandler = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(200, store, 'Store suspended successfully')
    );
});
