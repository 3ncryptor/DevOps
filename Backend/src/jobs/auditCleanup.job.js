 ...
 */
export const cleanupAuditLogs = async (retentionDays = DEFAULT_RETENTION_DAYS) => {
  ...
  try {
    ...
    if (countBefore === 0) {
      ...
      logJob.completed("AuditCleanup", Date.now() - startTime, { deleted: 0 });
      return { deleted: 0 };
    }
    ...
    while (true) {
      ...
      logger.debug(`Deleted batch: ${result.deletedCount}, Total: ${totalDeleted}`);
 ...
 */
export const archiveAndCleanup = async (retentionDays = DEFAULT_RETENTION_DAYS) => {
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - retentionDays);
