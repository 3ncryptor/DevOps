...
export const generateAllStoreMetrics = async (date = new Date()) => {
  ...
  logger.info(`Starting metrics generation for ${targetDate.toISOString().split('T')[0]}`);
  ...
  try {
    ...
    logger.info(`Found ${stores.length} active stores for metrics generation`);
    ...
    for (let i = 0; i < stores.length; i += batchSize) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(store => generateDailyMetrics(store._id, targetDate))
      );
    ...
    if (results.errors.length > 0) {
      logger.warn("Some stores failed metrics generation", { errors: results.errors });
    }
