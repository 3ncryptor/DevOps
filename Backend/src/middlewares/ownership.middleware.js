...
const checkOwnership = ({
  ...
  return async (req, _, next) => {
    ...
    if (ownerId !== userId) {
      throw new ApiError(403, `You don't have permission to access this ${resourceName.toLowerCase()}`);
    }
...
const checkSellerOwnership = async (req, _, next) => {
  ...
  if (!seller) {
    throw new ApiError(403, "Seller profile not found. Please complete seller registration.");
  }
...
const checkStoreOwnership = async (req, _, next) => {
  ...
  if (req.user.role === ROLES.SUPER_ADMIN) {
    if (req.params.storeId) {
      const store = await Store.findById(req.params.storeId).populate("sellerId");
      if (!store) {
  ...
  if (store.sellerId.toString() !== seller._id.toString()) {
    throw new ApiError(403, "You don't have permission to access this store");
  }
...
const checkProductOwnership = async (req, _, next) => {
  ...
  if (product.storeId.sellerId.toString() !== seller._id.toString()) {
    throw new ApiError(403, "You don't have permission to access this product");
  }
...
const checkOrderAccess = async (req, _, next) => {
  ...
  if (req.user.role === ROLES.SELLER) {
    const seller = await Seller.findOne({ userId: req.user.id });
    if (seller && order.storeId.sellerId.toString() === seller._id.toString()) {
      req.order = order;
      req.seller = seller;
...
const requireApprovedSeller = async (req, _, next) => {
  ...
  const seller = await Seller.findOne({ userId: req.user.id });
  if (!seller) {
    throw new ApiError(403, "Seller profile not found. Please complete registration.");
  }

  if (seller.status !== "APPROVED") {
    throw new ApiError(403, "Your seller account is pending approval or has been suspended");
  }
